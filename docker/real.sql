...
DELIMITER $$ CREATE TRIGGER auto_generate_id before
insert
  on usr_smile FOR EACH ROW BEGIN
SET
...
CREATE TABLE post (
    ...
    PRIMARY KEY (id),
    FOREIGN KEY (author_id) REFERENCES usr_smile (user_id) ON UPDATE CASCADE ON DELETE
    SET
      NULL
  );
CREATE TABLE category_node(
    ...
    postId INT NOT NULL,
    PRIMARY KEY (id),
    FOREIGN KEY (categoryId) REFERENCES category (id) ON UPDATE CASCADE ON DELETE
    SET
      NULL,
      FOREIGN KEY (postId) REFERENCES post (id) ON UPDATE CASCADE ON DELETE CASCADE
  );
DELIMITER $$ CREATE TRIGGER set_default_if_category_null
AFTER
  DELETE ON category FOR EACH ROW BEGIN
UPDATE
  category_node
SET
  categoryId = 0
...
DELIMITER $$ CREATE TRIGGER push_embed_if_status_publish
AFTER
INSERT
...
# push rating after publish post
DELIMITER $$ CREATE TRIGGER push_rating_if_status_publish
AFTER
INSERT
  ON post FOR EACH ROW BEGIN
INSERT INTO
...
DELIMITER $$ CREATE TRIGGER push_contrib
AFTER
UPDATE
  ON contrib_post_temp FOR EACH ROW BEGIN IF (new.accepted = TRUE) THEN
INSERT INTO
...
CREATE TABLE comment (
  ...
  PRIMARY KEY (id),
  FOREIGN KEY (userId) REFERENCES usr_smile (user_id) ON UPDATE CASCADE ON DELETE
  SET
    NULL,
    FOREIGN KEY (postId) REFERENCES post (id) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (reply_for_id) REFERENCES comment (id) ON DELETE CASCADE ON UPDATE CASCADE
);
# trigger for increment rating.comment if comment insert to post
DELIMITER $$ CREATE TRIGGER increment_rating_comment
AFTER
INSERT
  ON comment FOR EACH ROW BEGIN DECLARE old_rating INT;
SET
  old_rating = (
    SELECT
      comment
...
WHERE
  postId = new.postId;
END $$ DELIMITER;
DELIMITER $$ CREATE TRIGGER decrement_rating_comment BEFORE DELETE ON comment FOR EACH ROW BEGIN DECLARE old_rating INT;
SET
  old_rating = (
    SELECT
      comment
